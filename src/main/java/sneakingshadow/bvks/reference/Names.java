package sneakingshadow.bvks.reference;

public final class Names
{
    public static final class Keys
    {
        public static final String CATEGORY = "keys.bvks.category";
        public static final String CHARGE = "keys.bvks.charge";
        public static final String RELEASE = "keys.bvks.release";
    }

    public static final class Blocks
    {
        public static final String DevilOre = "devil_ore";
        public static final String DevilCobblestone = "devil_cobblestone";
        public static final String SmoothObsidian = "smooth_obsidian";
        public static final String DebugBlock = "debug_block";
    }

    public static final class Items
    {
        public static final String AssassinsKnife = "assassins_knife";
        public static final String DevilArrow = "devil_arrow";
        public static final String DevilAxe = "devil_axe";
        public static final String DevilHoe = "devil_hoe";
        public static final String DevilBoots = "devil_boots";
        public static final String DevilBow = "devil_bow";
        public static final String DevilChestplate = "devil_chestplate";
        public static final String DevilGem = "devil_gem";
        public static final String DevilHammer = "devil_hammer";
        public static final String DevilHelmet = "devil_helmet";
        public static final String DevilIngot = "devil_ingot";
        public static final String DevilLeggings = "devil_leggings";
        public static final String DevilPickaxe = "devil_pickaxe";
        public static final String DevilShovel = "devil_shovel";
        public static final String DevilSword = "devil_blade";
        public static final String DiamondHammer = "diamond_hammer";
        public static final String GoldHammer = "gold_hammer";
        public static final String IronHammer = "iron_hammer";
        public static final String IronRod = "iron_rod";
        public static final String ObsidianAxe = "obsidian_axe";
        public static final String ObsidianBoots = "obsidian_boots";
        public static final String ObsidianChestplate = "obsidian_chestplate";
        public static final String ObsidianHammer = "obsidian_hammer";
        public static final String ObsidianHelmet = "obsidian_helmet";
        public static final String ObsidianHoe = "obsidian_hoe";
        public static final String ObsidianIngot = "obsidian_ingot";
        public static final String ObsidianLeggings = "obsidian_leggings";
        public static final String ObsidianPickaxe = "obsidian_pickaxe";
        public static final String ObsidianRod = "obsidian_rod";
        public static final String ObsidianShovel = "obsidian_shovel";
        public static final String ObsidianSword = "obsidian_sword";
        public static final String StoneHammer = "stone_hammer";
        public static final String WoodHammer = "wood_hammer";
        public static final String DebugItem = "debug_item";
        public static final String DevilCompound = "devil_compound";
        public static final String AdminHammer = "admin_hammer";
        public static final String BottomlessVoid = "bottomless_void";
    }

    public static final class ArmorTextures
    {
        public static final String Devil = "devil_armor";
        public static final String Obsidian = "obsidian_armor";
    }

    public static final class CreativeTabs
    {
        public static final String Item = Ref.RESOURCE_PREFIX + "item";
        public static final String Block = Ref.RESOURCE_PREFIX + "block";
        public static final String Background = Ref.MOD_ID+".png";
    }

    public static final class Configuration
    {

    }
}
